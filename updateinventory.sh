...
declare -a StringArray=("db" "web")
if [[ $1 = "Ubuntu" ]]; then
    for VM in "${StringArray[@]}"; do 
      ipd=$(VBoxManage guestproperty enumerate $VM | grep IP |awk -F\. '{print $4}'|sed 's/\([0-9][0-9][0-9]\).*/\1/')
      sed -i '' -e 's/\(^'"$VM"' ansible_host=192\.168\.10\.\)[0-9]*[ ]*a/\1'"$ipd"' a/' inventory.txt
    done
